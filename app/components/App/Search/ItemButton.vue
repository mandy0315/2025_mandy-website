...
<template>
  <button @click="$emit('handleToPage')"
    class="flex group rounded mb-1 items-center text-left border-none w-full p-1 hover:bg-primary/20 text-sm lg:text-base">
    <div class="w-4">
      <Icon :name="icon" class="text-gray-500 w-full mt-1 group-hover:dark:text-white" />
    </div>
    <div v-if="category" class="ml-1 text-sm px-1 bg-primary/30 text-primary">{{ category }}</div>
